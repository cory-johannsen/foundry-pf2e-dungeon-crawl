...
describe("mcp-server relay wrappers", () => {
  ...
  it("getPendingTreasureCustomization sends the module.api call with the given sceneId", async () => {
    const fetchImpl = fakeFetch({
      roomId: "room-1",
      name: "The Locked Strongbox",
    });
    const result = await getPendingTreasureCustomization("scene-1", {
      baseUrl: "http://localhost:9999",
      apiKey: "k",
      clientId: "abc",
      fetchImpl,
    });
    expect(result).toEqual({ roomId: "room-1", name: "The Locked Strongbox" });
    const [, options] = fetchImpl.mock.calls[0];
    const script = JSON.parse(options.body).script;
    expect(script).toContain('getPendingTreasureCustomization("scene-1")');
  });

  it("applyTreasureCustomization sends sceneId, roomId, and the customization payload", async () => {
    const fetchImpl = fakeFetch({ sceneId: "scene-1", roomId: "room-1" });
    const result = await applyTreasureCustomization(
      ...
      "room-1",
      {
        ...
        summary: "A low cairn of fitted stones.",
      },
      {
        baseUrl: "http://localhost:9999",
        apiKey: "k",
    ...
    const [, options] = fetchImpl.mock.calls[0];
    const script = JSON.parse(options.body).script;
    expect(script).toContain('applyTreasureCustomization("scene-1", "room-1"');
    expect(script).toContain("The Cairn of the Unnamed");
  });
...
describe("mcp-server tool registration (full round-trip over an in-memory transport)", () => {
  ...
  it("submit_treasure_customization applies the customization via the relay", async () => {
    globalThis.fetch = vi.fn().mockResolvedValue({
      json: async () => ({ result: { sceneId: "scene-1", roomId: "room-1" } }),
    });
    const client = await connectedClient();
    const result = await client.callTool({
      name: "submit_treasure_customization",
      arguments: {
        sceneId: "scene-1",
        roomId: "room-1",
        name: "The Locked Strongbox",
    ...
    const [, options] = globalThis.fetch.mock.calls[0];
    const script = JSON.parse(options.body).script;
    expect(script).toContain("The Locked Strongbox");
    expect(script).toContain("its lock never picked");
  });
});
