...
how the ~40 files under `scripts/` and `tools/agent-loop/` relate to each
...
**Combat automation — two separate paths, not a hierarchy:**
- *In-module heuristic* (`dungeon-combat.mjs`, `combat-rewards.mjs`,
  `agent-candidates.mjs`, `dungeon-strike-riders.mjs`,
  `dungeon-critical-deck.mjs`) — wires a spawned encounter into a real
  PF2e `Combat`, picks agent-controlled combatants' turns via pure
  candidate-scoring logic, and auto-applies whatever Critical Hit/Fumble
  Deck directives parse cleanly.
- *External LLM agent-loop* (`tools/agent-loop/*`) — a standalone Node
  process that polls a live Foundry world over the `foundry-rest` relay
  and asks a configured LLM provider (Claude by default) which candidate
  to take, entirely outside the Foundry module's own process. Deliberately
  has no hardcoded LLM dependency inside the module itself.
...
`scripts/` and `tools/agent-loop/` — regenerate and paste over the block
below any time the graph might have changed, rather than hand-editing it.

```mermaid
graph LR
  subgraph "Agent-loop (external LLM combat AI)"
    tools_agent_loop_foundry_client_mjs["agent-loop/foundry-client.mjs"]
    tools_agent_loop_mcp_server_mjs["agent-loop/mcp-server.mjs"]
    tools_agent_loop_poll_mjs["agent-loop/poll.mjs"]
    tools_agent_loop_providers_claude_mjs["agent-loop/providers/claude.mjs"]
    tools_agent_loop_providers_index_mjs["agent-loop/providers/index.mjs"]
    tools_agent_loop_providers_laya_mjs["agent-loop/providers/laya.mjs"]
    tools_agent_loop_watch_pending_mjs["agent-loop/watch-pending.mjs"]
  end
  ...
  tools_agent_loop_mcp_server_mjs --> tools_agent_loop_foundry_client_mjs
  tools_agent_loop_poll_mjs --> tools_agent_loop_foundry_client_mjs
  tools_agent_loop_poll_mjs --> tools_agent_loop_providers_index_mjs
  tools_agent_loop_providers_claude_mjs --> tools_agent_loop_foundry_client_mjs
  tools_agent_loop_providers_index_mjs --> tools_agent_loop_foundry_client_mjs
  tools_agent_loop_providers_index_mjs --> tools_agent_loop_providers_claude_mjs
  tools_agent_loop_providers_index_mjs --> tools_agent_loop_providers_laya_mjs
  tools_agent_loop_providers_laya_mjs --> tools_agent_loop_foundry_client_mjs
  tools_agent_loop_watch_pending_mjs --> tools_agent_loop_foundry_client_mjs
  tools_agent_loop_watch_pending_mjs --> tools_agent_loop_mcp_server_mjs
```

Notably, `tools/agent-loop/*` never imports anything from `scripts/` — the
external agent-loop process talks to a live Foundry world only over HTTP
via the `foundry-rest` relay, never by sharing code with the module it's
driving.
